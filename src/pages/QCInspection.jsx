...
const status = (key, el) => {
  ...
  return (
    <button
      className={`flex items-center justify-center gap-1 w-full ${key == 'qcStatus' ? bg : ''} ${color} px-4 py-1.5 rounded-md text-md`}
    >
      {key == 'qcStatus' ? 'QC ' : ''}
      {val}
    </button>
...
export default function QCInspection() {
  ...
  return (
    <>
      <h2 className="fs-1 font-bold text-blue-900">QC Inspection</h2>
      <div className="flex flex-wrap items-center justify-between gap-3 mt-5">
        ...
        <button type="button" className="px-2 md:px-5 py-1 md:py-2 rounded-md text-white bg-[#EBEBEB] text-md font-medium active:shadow-inner transition">
          QC Inspection
        </button>
      </div>
